(function () {
  ...
  function buildCardsHtml(entries) {
    return entries.map(function (entry, idx) {
      ...
      var photoRowHtml = "";
      if (photos.length > 0) {
        var mainPhoto = '<div class="photo-main" data-full="' + photos[0] + '"><img src="' + photos[0] + '" alt="" /></div>';
        var thumbsHtml = "";
        if (photos.length > 1) {
          var thumbs = photos.slice(1, 3).map(function (p) {
            return '<div class="photo-thumb" data-full="' + p + '"><img src="' + p + '" alt="" /></div>';
          }).join("");
          var extra = photos.length - 3;
          if (extra > 0) {
            thumbs += '<div class="photo-thumb more">+' + extra + '</div>';
          }
  ...
  function attachCardEvents(container) {
    container.querySelectorAll("[data-full]").forEach(function (el) {
